...
class UtilAccess:
    ...
    def GetLanguageVersion():
        os_name = UtilAccess.adapter.get_os_name()
        if os_name == OS_Window:
            return "LANGUAGEVERSION=402"
        elif os_name == OS_Linux:
            return "LANGUAGEVERSION=502"
        elif os_name == OS_Mac:
            return "LANGUAGEVERSION=602"
...
class c:
    class EmQuantData:
        ...
        def resolve25RankDataEx(self, indicatorData, **arga):
            ...
            for i in range(0, len(self.Codes)):
                stockCode = self.Codes[i]
                if not self.Data.has_key(stockCode):
                    self.Data[stockCode] = []
                thislist = []
                for j in range(0, len(self.Indicators)):
                    tempData = None
        ...
        def resolve(self, variant):
            if variant.vtype == eVT_null:
                ...
                return unicode("".join(variant.eqchar.pChar), c.EncodeType)
    ...
    def geterrstring(cls, errcode, lang=1):
        ...
        :return:
        """
        return cls.__Exec("geterrstring", errcode, lang)
